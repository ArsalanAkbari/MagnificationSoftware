...
{
    public class MagnifierWindowsBothEyeAndHead : IDisposable
    {
        private Form form;
        ...
        ~MagnifierWindowsBothEyeAndHead()
        {
            Dispose(false);
        }
        ...
        public virtual void UpdateMaginifier()
        {
            ...
            //*************************Take Eye Gaze From Both Eye and Head**********************************************//
            ...
            if (overlayEyeNewForm.getFreezeMode() == false)
            {
                ...
                Point warpPointEyes = combineEyeGaze.GetNextPoint(gazePointEyes);

                //Console.WriteLine(combineEyeGaze.hasNoGaze);


                //
                Point targetEyes;
                Vector3 headPoint;
                Point warpPoint = new Point(0, 0);

                if (combineEyeGaze.hasNoGaze == false)
                {

                    targetEyes = calculateTargetPoint(warpPointEyes);
                    headPoint = headPoints.GetHeadPoint();
                    warpPoint = headPoints.GetNextPoint(targetEyes);
                }

                if (combineEyeGaze.hasNoGaze == true)
                {

                    targetEyes = calculateTargetPoint(warpPointEyes);
                    headPoint = headPoints.GetHeadPoint();
                    warpPoint = headPoints.GetNextPoint(new Point(overlayEyeNewForm.Width / 2, overlayEyeNewForm.Height / 2));
                }
        ...
        {
            RECT winRect;
            NativeMethods.GetWindowRect(hwndMag, out winRect);
