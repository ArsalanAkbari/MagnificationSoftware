...
{
    public class MagnifierWindowsBothEye : IDisposable
    {
        private Form form;
        ...
        void timer_Tick(object sender, EventArgs e)
        {
            ...
            UpdateMaginifier(overlayEyeNewForm.Handle);
            //overlayEyeNewForm.AdjustMasterWinPositionToAttachScreenEdge();

            if (overlayEyeNewForm.fullScreen == false)  //if window mode
                        overlayEyeNewForm.HandleTimerX();
        ...
        public Point calculateTargetPoint(Point gazePoint)
        {
            ...
            //Point retTarget = new Point(0, 0 );
            retTarget = new Point(0, 0);
            float slowMargin = 0.25f;
           ...
           // retTarget = lastTarget;
            retTarget = overlayEyeNewForm.mCurrentPoint;

            if (gazePoint.X > noMoveArea.right - width * fastMargin)
                retTarget.X += (int)(Math.Abs((gazePoint.X - (noMoveArea.right - width * slowMargin))) / m_MAGFACTOR );
            else if (gazePoint.X > noMoveArea.right - width * slowMargin)
                retTarget.X += (int)(Math.Abs((gazePoint.X - (noMoveArea.right - width * slowMargin))) / m_MAGFACTOR / m_MAGFACTOR );

            if (gazePoint.X < noMoveArea.left + width * fastMargin)
                retTarget.X += -(int)(Math.Abs((gazePoint.X - (noMoveArea.left + width * slowMargin))) / m_MAGFACTOR );
            else if (gazePoint.X < noMoveArea.left + width * slowMargin)
                retTarget.X += -(int)(Math.Abs((gazePoint.X - (noMoveArea.left + width * slowMargin))) / m_MAGFACTOR / m_MAGFACTOR);

            if (gazePoint.Y > noMoveArea.bottom - height * fastMargin)
                retTarget.Y += (int)(Math.Abs((gazePoint.Y - (noMoveArea.bottom - height * slowMargin))) / m_MAGFACTOR );
            else if (gazePoint.Y > noMoveArea.bottom - height * slowMargin)
                retTarget.Y += (int)(Math.Abs((gazePoint.Y - (noMoveArea.bottom - height * slowMargin))) / m_MAGFACTOR / m_MAGFACTOR);

            if (gazePoint.Y < noMoveArea.top + height * fastMargin)
                retTarget.Y += -(int)(Math.Abs((gazePoint.Y - (noMoveArea.top + height * slowMargin))) / m_MAGFACTOR );
            else if (gazePoint.Y < noMoveArea.top + height * slowMargin)
                retTarget.Y += -(int)(Math.Abs((gazePoint.Y - (noMoveArea.top + height * slowMargin))) / m_MAGFACTOR / m_MAGFACTOR );
        ...
        public  void UpdateMaginifier(IntPtr myhWnd)
        {
           ...
           // Point target2 = calculateTargetPoint(warpPoint);
